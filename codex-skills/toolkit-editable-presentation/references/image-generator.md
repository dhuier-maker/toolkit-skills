...
| Academic / professional feel | `suitable for academic publication`, `professional toolkit-diagram-generator` |
| Light background | `white background` or `light gray background` |

**Template**: `{toolkit-diagram-generator type} toolkit-diagram-generator showing {content description}, {component description} connected by {connection method}, {style} style with {color scheme}, white background, clear labels, professional technical toolkit-diagram-generator`
...
- First `.env` found in this order: current working directory, clone repo root, `~/.toolkit-editable-presentation/.env`
