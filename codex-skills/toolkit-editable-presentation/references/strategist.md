...
> 4. Search for each concept's filename in the chosen library: `ls skills/toolkit-editable-presentation/templates/icons/<chosen-library>/ | grep <keyword>`
...
**When selection includes B**, you must run `python scripts/analyze_images.py <project_path>/images` before outputting the spec, and integrate scan results into the image resource list.
