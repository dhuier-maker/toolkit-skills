...
2. First `.env` found in this order:
   ...
   - Clone repo root
   - `~/.toolkit-editable-presentation/.env`
...
`image_search.py` uses the same `.env` lookup order as `image_gen.py`, so skill installs can keep `PEXELS_API_KEY` / `PIXABAY_API_KEY` in `~/.toolkit-editable-presentation/.env`.
