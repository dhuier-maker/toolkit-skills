...
> 基于 Anthropic 官方三支产品动画（Codex Design / Codex Desktop / Codex for Word）
...
- Codex Design 展示 tweak 参数、拖滑块（不是一键生成完美结果）
- Codex 展示代码报错 + AI 修复（不是一次成功）
- Codex for Word 展示 Redline 红删绿增的修改过程（不是直接给最终稿）
...
### 配方 A · Apple Keynote 戏剧式（Codex Design 类）
...
- Anthropic 官方动画拆解：设计者项目目录的 `参考动画/BEST-PRACTICES.md`
