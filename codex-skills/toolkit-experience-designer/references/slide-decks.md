...
showcase grammar 确认后、批量推剩余页面前，必须先产出 `outline.md`。这是整套 deck 的"剧本"——先确认逻辑，再生成图片，效率最高。
...
| # | 标题 | 页面类型 | 叙事目标 | 视觉方案 |
|---|------|---------|---------|---------|
| 1 | 封面 | title slide | 开场定调 | 全幅品牌色 + logo |
| 2 | Agenda | agenda | 告知结构 | 白底 + 章节列表 |
| 3 | 痛点陈述 | problem | 引发共鸣 | 大字引语 + 暗调背景 |
| 4 | 数据印证 | data | 建立可信 | 大数字 + 趋势折线 |
...
用户确认 outline.md 后再批量生成全 deck。期间若要调整某页，直接改 outline.md 对应行即可。
---
...
原因：ESM 模块解析从脚本所在位置向上找 `node_modules`。脚本在 `~/.claude/skills/toolkit-experience-designer/scripts/`，那里没依赖。
...
**幻灯片制作完成后自动生成**，无需用户手动要求。生成 `speaker-notes.md` 并写入 PPTX 备注区。生成后用户可手动修改，修改后重新合成 PPTX 即可同步。

加了 speaker notes 你就可以把 slide 上的文字减少到最小，focus on impactful visuals——notes 承载完整 script。

### 自动生成规则

- **触发时机**：所有 slide 页面制作完成后、导出 PPTX 之前自动执行
- **输出文件**：`{deck目录}/speaker-notes.md`
- **每页内容**：200-400 字对话式口播稿，分 3 段——本页要点 → 关键价值 → 与下一页衔接语
- **情绪标注**：[停顿]、[重音]、[强调] 等口播提示

### 格式

**speaker-notes.md**（数组格式，第 N 项对应第 N 张 slide）：

```markdown
## Slide 1 - 封面
[停顿2秒] 各位好，今天向大家介绍[重音]XX产品的核心价值。
[停顿] 我们先来看一组数据...
[下一页衔接] 看到这些数据，你可能会问——为什么增长这么快？

## Slide 2 - 痛点
...
```

**写入 PPTX 备注区**：`export_deck_pptx.mjs` 自动将 `speaker-notes.md` 对应内容写入每页备注。
