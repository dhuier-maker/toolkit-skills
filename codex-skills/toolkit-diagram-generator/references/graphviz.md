...
**Engine**: DOT language | **Code Fence**: ```dot

> **Important:** Use ```dot as the code fence identifier, NOT ```graphviz.

## Quick Start

Choose `digraph` (directed) or `graph` (undirected) → Define nodes with attributes → Connect with `->` or `--` → Set layout → Wrap in ```dot fence.

Default: top-to-bottom (`rankdir=TB`), cluster names must start with `cluster_`, use semicolons.
...
See [references/syntax.md](../graphviz/references/syntax.md) for layout control (rankdir, splines, rank), HTML labels, edge styles, cluster subgraphs, and record-based nodes.
