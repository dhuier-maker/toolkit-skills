...
Use an absolutely-positioned SVG element overlaid on the toolkit-diagram-generator container. Components get `id` attributes, and SVG `<path>` elements with orthogonal segments (horizontal + vertical only) connect them visually.
...
1. **Container setup** — Wrap the toolkit-diagram-generator content in a `<div style="position: relative;">`, then place the `<svg>` overlay as the last child
