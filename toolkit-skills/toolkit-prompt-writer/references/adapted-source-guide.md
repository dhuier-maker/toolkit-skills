...
**Codex (claude.ai, Codex API, Codex 4.x)**
- Be explicit and specific — Codex follows instructions literally, not by inference
- XML tags help for complex multi-section prompts: `<context>`, `<task>`, `<constraints>`, `<output_format>`
- Codex Opus 4.x over-engineers by default — add "Only make changes directly requested. Do not add features or refactor beyond what was asked."
- Provide context and reasoning WHY, not just WHAT — Codex generalizes better from explanations
...
- Be more explicit than you would with Codex or GPT — instruction following is weaker
...
- Codex Opus 4.x over-engineers — add "Only make changes directly requested. Do not add extra files, abstractions, or features."
...
**Cline (formerly Codex Dev)**
- Agentic VS Code extension — autonomously edits files, runs terminal commands, uses browser tools
- Powered by Codex, GPT, or other LLMs — prompting style should match the underlying model
...
**Computer-Use / Browser Agents** (Perplexity Comet/Computer, OpenAI Atlas, Codex in Chrome, OpenClaw Agents)
...
**Chain of Thought** — for logic, math, and debugging on standard reasoning models ONLY (Codex, GPT-5.x, Gemini, Qwen2.5, Llama). Never on o3/o4-mini/R1/Qwen3-thinking.
...
1. **禁止伪造技术** — Mixture of Experts、Tree of Thought、Graph of Thought 在单 prompt 执行中是伪造的（模型模拟多路径但无真实并行），绝对不要嵌入
2. **推理模型不加 CoT** — o3/o4-mini/DeepSeek-R1/Qwen3 thinking 模式内部已含推理链，显式添加 CoT 反而降低输出质量
3. **最多 3 个澄清问题** — 超过 3 个问题应基于上下文推断，直接产出 prompt
4. **输出必须可粘贴** — 最终产物是单个可复制 prompt 块，不附带理论解释（除非用户要求）
5. **关键约束前置** — 最重要的约束放在 prompt 前 30%，对抗注意力衰减
