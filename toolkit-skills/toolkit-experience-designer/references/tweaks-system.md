...
**跨 agent 环境适配**：某些 design-agent 原生环境（如 Codex.ai Artifacts）依赖 host 的 postMessage 把 tweak 值回写源码做持久化。本 skill 采用**纯前端 localStorage 方案**——效果一致（刷新保留状态），但持久化发生在浏览器 localStorage 而不是源码文件。这个方案在任何 agent 环境（Codex / Codex / Cursor / Trae / etc.）都能工作。
...
如果你以后想把设计上传到支持源码级 tweaks（如 Codex.ai Artifacts）的环境也能跑，保留 **EDITMODE 标记块**：
