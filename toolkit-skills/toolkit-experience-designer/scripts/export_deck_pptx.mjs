...
/**
 * 解析 speaker-notes.md，提取每页对应的备注。
 * 格式：## Slide N - 标题\n\n内容...\n\n## Slide N+1 ...
 * 返回数组，第 N 项对应第 N 张 slide。
 */
async function loadSpeakerNotes(deckDir) {
  const notesPath = path.join(deckDir, 'speaker-notes.md');
  let raw;
  try {
    raw = await fs.readFile(notesPath, 'utf-8');
  } catch {
    return []; // 无 speaker-notes.md，返回空数组
  }

  // 按 ## Slide N 分割，取 "## Slide N" 之后、"## Slide N+1" 之前的内容
  const notes = [];
  const sections = raw.split(/(?=^## Slide \d)/m);
  for (const section of sections) {
    const trimmed = section.trim();
    if (!trimmed) continue;
    // 去掉 "## Slide N - 标题" 这行，保留正文
    const lines = trimmed.split('\n');
    const bodyLines = lines.filter((l, i) => i > 0 && !l.trim().startsWith('['));
...
async function main() {
  ...
  // 尝试加载 speaker-notes.md
  const speakerNotes = await loadSpeakerNotes(slidesDir);
  if (speakerNotes.length > 0) {
    console.log(`Loaded ${speakerNotes.length} speaker notes from speaker-notes.md`);
  }
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      const { slide } = await html2pptx(fullPath, pres);
      // 写入备注（如果存在对应条目）
      if (speakerNotes[i]) {
        slide.addNotes(speakerNotes[i]);
      }
      console.log(`  [${i + 1}/${files.length}] ${f} ✓${speakerNotes[i] ? ' +notes' : ''}`);
    } catch (e) {
  ...
  console.log(`\n✓ Wrote ${outFile}  (${files.length - errors.length}/${files.length} slides, 可编辑 PPTX)${speakerNotes.length > 0 ? ' + 备注' : ''}`);
}
