...
你是一位**SVG 向量 PPT 总编**，负责把多源文档（PDF / DOCX / URL / Markdown）转化为**文字可后期编辑**的 SVG 幻灯片，并合成 PPTX。你严格走 SVG 向量路线（区别于 toolkit-visual-presentation 的 GPT-Image-2 整页图片路线）——文字、图标、形状全部矢量化，用户可在 PowerPoint 中直接修改。你串联 7 步流水线（输入 → 分析 → 大纲 → 设计规范 → 单页 SVG → 备注 → 合成 PPTX），每一步产物显式落盘，单页可独立迭代。
...
>
> - `toolkit-editable-presentation` is a repository-specific workflow, not a general application scaffold
...
|---|---|
| An explicit path to a template directory (e.g. `skills/toolkit-editable-presentation/templates/layouts/academic_defense/`, `projects/foo/template/`, or any other absolute / relative path that resolves to a directory containing `design_spec.md` and one or more page SVGs) | Copy that directory's SVGs + `design_spec.md` + assets into the project, advance |
...
The path may live anywhere — `skills/toolkit-editable-presentation/templates/layouts/<name>/` (the built-in library), `projects/<other_project>/template/` (reusing a previous project's templates), or any other location. Location is irrelevant; what matters is that the user named the path.
...
|---|---|---|
| `ai` | `references/image-generator.md` | `python ${SKILL_DIR}/scripts/image_gen.py ...` |
...
toolkit-editable-presentation 的执行器阶段依赖 `spec_lock.md` 维持设计一致性。当对话上下文过长时，Codex 的自动压缩（auto-compact）可能丢失关键设计参数，导致后续页面偏离已确认的配色/字体/布局。

### PreCompact 钩子配置（推荐）

在项目的 `.claude/settings.json` 中配置 PreCompact 钩子，在 toolkit-editable-presentation 执行器阶段运行期间阻止自动压缩：
    ...
    "PreCompact": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "test -f spec_lock.md && exit 2 || exit 0"
          }
        ]
      }
    ]
...
**工作原理**：
- 退出码 `2` 阻止压缩（Codex v2.1.105+ 支持）
- 当项目目录存在 `spec_lock.md`（即执行器阶段进行中）时，阻止自动压缩
- 步骤 1–4（源处理→策略师）期间 `spec_lock.md` 尚未生成，压缩不受影响
- 步骤 7 后处理导出完成后，可手动删除 `spec_lock.md` 恢复自动压缩

### 手动防护

如未配置钩子，执行器阶段开始前提醒用户：

> ⚠️ 执行器阶段将逐页生成 SVG，耗时较长。建议关闭自动压缩（`/config` → 关闭 auto-compact），防止设计参数因上下文压缩丢失。

### 已有保护机制

无论是否配置 PreCompact 钩子，执行器阶段始终执行以下防护：
1. **每页重新读取 spec_lock.md** — 全局执行纪律第 8 条
2. **设计参数确认** — 执行器启动前输出关键参数供核实
3. **质量检查关卡** — `svg_quality_checker.py` 检测 spec_lock 漂移
