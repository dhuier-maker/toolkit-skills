...
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project> -t push --transition-duration 0.6

# Disable
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project> -t none

# Auto-advance every 5 seconds (kiosk-style playback)
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project> --auto-advance 5
```
...
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project>

# Disable per-element animation entirely
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project> -a none

# Use a single effect (still cascades via the default after-previous trigger)
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project> --animation fade

# Switch to on-click for live presentations (presenter controls pacing)
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project> --animation-trigger on-click

# Custom pacing
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project> --animation mixed \
        --animation-stagger 0.7 --animation-duration 0.5

# All groups animate in unison on slide entry
python skills/toolkit-editable-presentation/scripts/svg_to_pptx.py <project> --animation-trigger with-previous
```
...
Executors should wrap logical sections in `<g id>` regardless of whether you plan to animate. The Executor reference (`skills/toolkit-editable-presentation/references/shared-standards.md`) requires it.
