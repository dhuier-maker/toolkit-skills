---
name: toolkit-test-coordinator
description: 协调前端测试、缺陷分类、修复和回归循环。适用于页面、组件、路由、浏览器兼容、构建和发布前验证，持续推进直到满足明确退出条件。
---
...
将问题区分为业务缺陷、工程基线阻塞、环境阻塞和非阻断警告。只有关键用户路径通过、严重缺陷关闭、构建成功且剩余限制已记录时才建议退出；lint、单测或浏览器验证未执行时，不能用构建成功替代。
