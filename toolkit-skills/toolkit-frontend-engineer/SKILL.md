...
description: 在现有仓库中实现、重构和验证可维护的生产级前端。适用于 Vue、React、组件架构、页面路由、状态管理、接口对接、数据大屏和响应式开发；不负责后端服务实现。
---
...
从 `package.json`、构建配置和源码入口确认框架及版本、目录分层、路由来源、状态管理、组件库、样式体系、接口封装、测试命令和构建输出。遗留工程不默认升级框架，不把 Vue 3、React Hooks 或 TypeScript 写法套入 Vue 2 项目。

实现时优先复用公共组件、别名、数据适配和错误处理约定。页面包含定时器、事件监听、图表或 WebSocket 时，必须验证销毁清理；固定比例大屏还需验证缩放和图表 resize。
